...
`contrib/moneytreexe-qt.pro` takes care of generating `.qm` (binary compiled) files from `.ts` (source files) files. It’s mostly automated, and you shouldn’t need to worry about it.
...
2. Update `src/qt/moneytreexe_locale.qrc` manually or via
...
To create a new language template, you will need to edit the languages manifest file `src/qt/moneytreexe_locale.qrc` and add a new entry. Below is an example of the English language entry.
